...
import (
	...
	"github.com/kaiachain/kaia/tests"
...
func runTracer(t *testing.T, tc *tracerTestdata, tracer vm.Tracer) (*types.Transaction, *blockchain.ExecutionResult, json.RawMessage) {
	...
	var (
		...
		statedb      = tests.MakePreState(database.NewMemoryDBManager(), alloc, config.Rules(new(big.Int).SetUint64(uint64(tc.Context.Number))))
