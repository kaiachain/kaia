...
// GetNodeByState retrieves the validators at the specified block filtered by state.
// If no states are provided, return all nodes
func (api *ValsetAPI) GetNodeByState(number *rpc.BlockNumber, states []valset.State) (map[string]any, error) {
	num, err := api.resolveRpcNumber(number, true)
	if err != nil {
		return nil, err
	}
	validators, err := api.vs.GetNodeByState(num, states)
	if err != nil {
		return nil, err
	}
