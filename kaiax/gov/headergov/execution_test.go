...
func TestPostInsertBlock(t *testing.T) {
	...
	vote, _ := NewVoteData(common.Address{1}, Params[GovernanceUnitPrice].Name, uint64(100)).Serialize()
	gov, _ := NewGovData(map[ParamEnum]interface{}{
...
func TestHandleVoteGov(t *testing.T) {
	...
	v := NewVoteData(common.Address{1}, Params[GovernanceUnitPrice].Name, uint64(100))
	require.NotNil(t, v)
	require.Equal(t, 0, len(h.cache.GroupedVotes()))
	require.Equal(t, 1, len(h.cache.Govs())) // gov at genesis
	voteBlock := uint64(5)
	...
	for range 2 {
		err := h.HandleGov(govBlock, NewGovData(map[ParamEnum]interface{}{
			GovernanceUnitPrice: uint64(100),
		}))
		assert.NoError(t, err)
		assert.Equal(t, 2, len(h.cache.Govs()))
