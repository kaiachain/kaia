...
func TestRewindTo_PurgesCache(t *testing.T) {
	...
	v.pfsCache.Add(uint64(1), map[common.Address]uint64{numToAddr(0): 1})
	v.pfsCache.Add(testCheckpointInterval+1, map[common.Address]uint64{numToAddr(0): 2})
	v.cpMatrixCache.Add(uint64(1), vrank.CPMatrix{numToAddr(1): {numToAddr(2): 1}})
	v.cpMatrixCache.Add(testCheckpointInterval+1, vrank.CPMatrix{numToAddr(1): {numToAddr(2): 2}})
...
func TestRewindDelete_UpdatesLastCheckpoint(t *testing.T) {
	...
	WriteCheckpoint(db, cp1,
		map[common.Address]uint64{numToAddr(0): 1},
		vrank.CPMatrix{numToAddr(1): {numToAddr(2): 1}},
	)
	WriteCheckpoint(db, cp2,
		map[common.Address]uint64{numToAddr(0): 2},
		vrank.CPMatrix{numToAddr(1): {numToAddr(2): 2}},
	)
...
func TestRewindDelete_MultiIntervalReorg(t *testing.T) {
	...
	WriteCheckpoint(db, cp1, map[common.Address]uint64{numToAddr(0): 1}, nil)
	WriteCheckpoint(db, cp2, map[common.Address]uint64{numToAddr(0): 2}, nil)
	WriteCheckpoint(db, cp3, map[common.Address]uint64{numToAddr(0): 3}, nil)
