...
Building the Kaia node binaries as well as utility tools, such as `kcn`, `ken`, `kbn`, `kscn`, `ksen`, `kgen`, `homi` and `abigen` requires
both a Go and a C compiler. You can install them using your favorite package manager. Once the dependencies are installed, run
    ...
    make all (or make {kcn, ken, kbn, kscn, ksen, kgen, homi, abigen})
