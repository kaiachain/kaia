...
StakingInfo is a data including stakingAmount and addresses (node, staking, reward, KIF and KEF).
...
stakingAddress are addresses of staking contracts of CCO. reward, KIF and KEF addresses are the addresses which get a block reward when a block has been created.
...
Kaia distributes the reward of a block to proposer, KIF and KEF.
The detail information of KIF and KEF is available on Kaia docs.
...
Second, divide totalReward by ratio (default 34/54/12 - proposer/KIF/KEF).
Last, distribute reward to each address (proposer, KIF, KEF).
