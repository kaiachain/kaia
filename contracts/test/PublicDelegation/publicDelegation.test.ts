...
import { addTime, augmentChai, DIRTY, toPeb, WEEK } from "../common/helper";
import { smock } from "@defi-wonderland/smock";
import { CnStakingV3MultiSig, CnStakingV3MultiSig__factory } from "../../typechain-types";
...
describe("PublicDelegation tests", function () {
  ...
  describe("Operations", function () {
    ...
    it("#updateCommissionTo: clean up reward to previous commissionTo address", async function () {
      ...
      expect(await ethers.provider.getBalance(commissionTo[0])).to.be.eq(before.add(toPeb(100n)));
    });
  ...
  describe("Simple delegation tests", function () {
    it("#stake: user can stake", async function () {
      ...
      expect(await ethers.provider.getBalance(cnV3s[0].address)).to.be.eq(toPeb(1000n));
      ...
      expect(await ethers.provider.getBalance(cnV3s[0].address)).to.be.eq(toPeb(3000n));
      ...
      expect(await ethers.provider.getBalance(cnV3s[0].address)).to.be.eq(toPeb(6000n));
    ...
    it("#stakeFor: user can't stake for zeor address", async function () {
      ...
      // Shares of user2 = 1000 * 10^18.
      await expect(
        pd1.connect(user1).stakeFor(ethers.constants.AddressZero, { value: toPeb(1000n) })
    ...
    it("#fallback: fallback leads to stakes", async function () {
      ...
      // User1 stakes 1000 KAIA.
      // Shares of user1 = 1000 * 10^18.
      await expect(user1.sendTransaction({ to: pd1.address, value: toPeb(1000n) }))
        .to.emit(pd1, "Staked")
        .withArgs(user1.address, toPeb(1000n), toPeb(1000n));
    ...
    it("#redeem: user can't withdraw to zero address", async function () {
      ...
      await expect(pd1.connect(user1).redeem(ethers.constants.AddressZero, toPeb(1000n))).to.be.revertedWith(
        "Address is null."
      );
    });
    ...
    it("#withdraw: user can't withdraw to zero address", async function () {
      ...
      await expect(pd1.connect(user1).withdraw(ethers.constants.AddressZero, toPeb(1001n))).to.be.revertedWith(
        "Address is null."
      );
    });
    ...
    it("#claim: user can withdraw after the lockup period", async function () {
      ...
      expect(await ethers.provider.getBalance(cnV3s[0].address)).to.be.eq(toPeb(0n));
      expect(await pd1.balanceOf(user1.address)).to.be.eq(0);
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(0);
      expect(await pd1.totalAssets()).to.be.eq(toPeb(0n));
      expect(await pd1.totalSupply()).to.be.eq(toPeb(0n));
  ...
  describe("Simple redelegation tests", function () {
    it("#redelegateByShares (redelegateByAssets): user can redelegate staked amount", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      // User1 redelegates 1000 KAIA to cnV3s[1].
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].address, toPeb(1000n)))
        ...
        .withArgs(user1.address, cnV3s[0].address, cnV3s[1].address, toPeb(1000n))
        .to.emit(pd2, "Staked");
      ...
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(toPeb(0n));
      // pd2:
      expect(await ethers.provider.getBalance(cnV3s[1].address)).to.be.eq(toPeb(1000n));
    ...
    it("#redelegateByShares (redelegateByAssets): shares must be calculated correctly", async function () {
      ...
      await pd1.connect(user1).redelegateByShares(cnV3s[1].address, toPeb(500n));
      ...
      await pd1.connect(user2).redelegateByAssets(cnV3s[1].address, toPeb(1000n));
    ...
    it("#redelegateByShares (redelegateByAssets): user can't redelegate if isRedelegationEnabled is false", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].address, toPeb(1000n))).to.be.revertedWith(
        "Redelegation disabled."
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[1].address, toPeb(1000n))).to.be.revertedWith(
        "Redelegation disabled."
      );
    });
    ...
    it("#redelegateByShares (redelegateByAssets): user can't redelegate more than staked amount", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[1].address, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
        "ERC20InsufficientBalance"
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[1].address, toPeb(1001n))).to.be.revertedWithCustomError(
        pd1,
        "ERC20InsufficientBalance"
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to same CnStakingV3", async function () {
      ...
      await expect(pd1.connect(user1).stake({ value: toPeb(1000n) })).to.emit(pd1, "Staked");
      await expect(pd1.connect(user1).redelegateByShares(cnV3s[0].address, toPeb(1000n))).to.be.revertedWith(
        "Target can't be self."
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV3s[0].address, toPeb(1000n))).to.be.revertedWith(
        "Target can't be self."
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate consecutive times", async function () {
      ...
      await addTime(WEEK / 2);
      await expect(pd2.connect(user1).redelegateByShares(cnV3s[0].address, toPeb(1000n))).to.be.revertedWith(
        "Can't redelegate yet."
      );
      await expect(pd2.connect(user1).redelegateByAssets(cnV3s[0].address, toPeb(1000n))).to.be.revertedWith(
        "Can't redelegate yet."
      );
      ...
      await expect(pd2.connect(user1).redelegateByShares(cnV3s[0].address, toPeb(1000n)))
        ...
        .withArgs(user1.address, cnV3s[1].address, cnV3s[0].address, toPeb(1000n))
        .to.emit(pd1, "Staked");
      ...
      expect(await pd1.maxRedeem(user1.address)).to.be.eq(toPeb(1000n));
      // pd2:
      expect(await ethers.provider.getBalance(cnV3s[1].address)).to.be.eq(toPeb(0n));
    ...
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to wrong CnStakingV3", async function () {
      ...
      await expect(pd1.connect(user1).redelegateByShares(cnV2.address, toPeb(1000n))).to.be.revertedWith(
        "Invalid CnStakingV3."
      );
      await expect(pd1.connect(user1).redelegateByAssets(cnV2.address, toPeb(1000n))).to.be.revertedWith(
        "Invalid CnStakingV3."
      );
    });
    it("#redelegateByShares (redelegateByAssets): user can't redelegate to CN not registered at address book", async function () {
      ...
      const fakeCnV3 = await smock.fake<CnStakingV3MultiSig>(CnStakingV3MultiSig__factory.abi);

      await expect(pd1.redelegateByShares(fakeCnV3.address, toPeb(1000n))).to.be.revertedWith("Invalid CnStakingV3.");
      await expect(pd1.redelegateByAssets(fakeCnV3.address, toPeb(1000n))).to.be.revertedWith("Invalid CnStakingV3.");
    });
  });
  ...
  describe("Scenario tests", function () {
    ...
    it("3 users stake and withdraw", async function () {
      ...
      // User1 withdraws 1000 shares.
      {
        ...
        expect((await cnV3s[0].staking()).toBigInt() - (await cnV3s[0].unstaking()).toBigInt()).to.be.eq(0n);
      }
    ...
    it("3 users stake and redelegate", async function () {
      ...
      {
        await pd1.connect(user1).redelegateByAssets(cnV3s[1].address, toPeb(1000n));
        ...
        await checkPSStates(
          pd1,
          [user1, user2, user3],
          [(await pd1.maxRedeem(user1.address)).toBigInt(), toPeb(1500n), toPeb(500n)]
        );
      ...
      {
        await pd1.connect(user2).redelegateByAssets(cnV3s[1].address, assetsForShares(pd2, toPeb(500n)));
      ...
      // User3 withdraws all shares.
      {
        ...
        await checkPSStates(
          pd1,
          [user1, user2, user3],
          [(await pd1.maxRedeem(user1.address)).toBigInt(), (await pd1.maxRedeem(user2.address)).toBigInt(), 0n]
        );
        await checkPSStates(pd2, [user1, user2, user3], [toPeb(1000n), toPeb(500n), 0n]);
      }
      ...
      // User2 withdraws all shares.
      {
        ...
        await checkPSStates(pd1, [user1, user2, user3], [(await pd1.maxRedeem(user1.address)).toBigInt(), 0n, 0n]);
        await checkPSStates(pd2, [user1, user2, user3], [toPeb(1000n), 0n, 0n]);
      }
      ...
      const commission = (await ethers.provider.getBalance(commissionTo[0])).toBigInt();
      expect(commission).to.be.eq(balanceCommissionTo1.toBigInt() + parseEther("4.48").toBigInt());

      const commission2 = (await ethers.provider.getBalance(commissionTo[1])).toBigInt();
      expect(commission2).to.be.eq(balanceCommissionTo2.toBigInt() + parseEther("2.56").toBigInt());
      ...
      for (let i = 0; i < cnV3s.length; i++) {
        expect(await ethers.provider.getBalance(cnV3s[i].address)).to.be.eq(toPeb(0n));
