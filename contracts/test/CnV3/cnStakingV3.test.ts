...
import { addTime, nowTime, setTime, WEEK } from "../common/helper";
  ...
  describe("Initial Lockup Enabled", function () {
    describe("CnStakingV3 setup", function () {
      it("Initial setup", async function () {
        ...
        expect(await cnV3.publicDelegation()).to.equal(ethers.constants.AddressZero);
      ...
      it("#setStakingTracker: Staking tracker should be valid", async function () {
        ...
        await expect(cnV3.setStakingTracker(ethers.constants.AddressZero)).to.be.reverted;
    ...
    describe("Ownable operations", function () {
      ...
      it("#updateStakingTracker: can't accept invalid staking tracker", async function () {
        ...
        await expect(cnV3.updateStakingTracker(fakeStakingTracker.address)).to.be.revertedWith(
    ...
    describe("withdrawLockupStaking", function () {
      ...
      it("#withdrawLockupStaking: can't withdraw to zero address", async function () {
        ...
        await setTime(unlockTime[0] + 100);
        await expect(cnV3.withdrawLockupStaking(ethers.constants.AddressZero, unlockAmount[0])).to.be.revertedWith(
          "Address is null."
        );
      });
      it("#withdrawLockupStaking: transfer failed if receiver is CA without fallback", async function () {
        ...
        await setTime(unlockTime[0] + 100);
        await expect(cnV3.withdrawLockupStaking(stakingTracker.address, unlockAmount[0])).to.be.revertedWith(
          "Transfer failed."
        );
      });
      ...
      it("#withdrawLockupStaking: can withdraw after unstakeTime", async function () {
        ...
        expect(await ethers.provider.getBalance(cnV3.address)).to.equal(300 - unlockAmount[0]);
      });
    });
    describe("Add free stakes", function () {
      ...
      it("#delegate: can't delegate if not initialized", async function () {
        ...
        await expect(deployer.sendTransaction({ to: cnV3.address, value: 100 })).to.be.revertedWith(
      ...
      it("#fallback: not allowed", async function () {
        ...
        await expect(deployer.sendTransaction({ to: cnV3.address, value: 100, data: "0x12345678" })).to.be.reverted;
      });
    });
    ...
    describe("Withdraw free stakes", function () {
      ...
      it("#approveStakingWithdrawal: can't set zero address", async function () {
        ...
        await expect(cnV3.approveStakingWithdrawal(ethers.constants.AddressZero, 10)).to.be.revertedWith(
          "Address is null."
        );
      });
  ...
  describe("Public Delegation Enabled", function () {
    ...
    describe("CnStakingV3 setup", function () {
      it("Initial setup", async function () {
        ...
        for (let i = 0; i < node.length; i++) {
          ...
          expect(await cnV3s[i].rewardAddress()).to.not.equal(ethers.constants.AddressZero);
          ...
          expect(await ethers.provider.getBalance(cnV3s[i].address)).to.equal(0);

          expect(await cnV3s[i].voterAddress()).to.equal(voterAddress[i]);
          expect(await cnV3s[i].stakingTracker()).to.equal(stakingTracker.address);
      ...
      it("#setPublicDelegation: commission rate should be less than max", async function () {
        ...
        await expect(cnV3s[0].setPublicDelegation(pdFactory.address, pdParam)).to.be.revertedWith(
      ...
      it("#setPublicDelegation: pd params should be set", async function () {
        ...
        await expect(cnV3s[0].setPublicDelegation(pdFactory.address, "0x")).to.be.revertedWith("Invalid args.");
      });
      it("#setPublicDelegation: pd factory can't be null address'", async function () {
        ...
        await expect(cnV3s[0].setPublicDelegation(ethers.constants.AddressZero, testingPsParam)).to.be.revertedWith(
          "Address is null."
        );
      });
      ...
      it("#depositLockupStakingAndInit: msg.value must be 0 if pd enabled", async function () {
        ...
        await expect(cnV3s[0].setPublicDelegation(pdFactory.address, pdParam)).to.emit(cnV3s[0], "SetPublicDelegation");
      ...
      it("#setPublicDelegation: only owner can set public delegation", async function () {
        ...
        await expect(
          cnV3s[0].connect(user1).setPublicDelegation(pdFactory.address, "0x1234")
      ...
      it("#setPublicDelegation: can't set PD if already initialized", async function () {
        ...
        await expect(cnV3s[0].setPublicDelegation(pdFactory.address, "0x1234")).to.be.revertedWith(
    ...
    describe("Delegation", function () {
      it("Delegation must be done through PublicDelegation to prevent malicious delegation", async function () {
        ...
        await expect(deployer.sendTransaction({ to: cnV3s[0].address, value: 100 })).to.be.revertedWithCustomError(
          cnV3s[0],
          "AccessControlUnauthorizedAccount"
        );
      });
      ...
      describe("Redelegation", function () {
        it("Redelegation must be done through PublicDelegation to prevent malicious delegation", async function () {
          ...
          await expect(cnV3s[0].redelegate(deployer.address, cnV3s[1].address, 100)).to.be.revertedWith(
            "Redelegation disabled."
          );
        });
        it("Redelegation target must be a valid CnStakingV3 registered at address book", async function () {
          ...
          const fakeCnV3 = await smock.fake<CnStakingV3>(CnStakingV3__factory.abi);

          await expect(
            cnV3s[0].connect(ps1Signer).redelegate(deployer.address, fakeCnV3.address, 100)
