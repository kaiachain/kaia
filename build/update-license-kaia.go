...
// Each file is checked in the following order.
// The top is high priority.
const (
	LICENSE_TYPE_KAIA = iota
	LICENSE_TYPE_KLAYTN
	LICENSE_TYPE_MIXED
	LICENSE_TYPE_ETHEREUM
	LICENSE_TYPE_GO
	LICENSE_TYPE_ANY
	LICENSE_TYPE_NONE
	NUM_LICENSE
...
type licenseReplacer struct {
	name   string
	pat    *regexp.Regexp // license pattern
	writer func(b []byte, matches []int, info *info) []byte
}
...
var (
	...
	no_klaytn_licenses = []string{""}

	// licenseReplacers must be iterated in the priority order because
	// there can be multiple license patterns that match the given file.
	licenseReplacers = map[int]licenseReplacer{
		LICENSE_TYPE_KAIA: {
			name:   "Kaia license",
			pat:    regexp.MustCompile(`//\s*((Modifications )?Copyright (?P<year>\d+) (The Kaia Authors))`),
			writer: nil,
		},
		LICENSE_TYPE_KLAYTN: {
			name: "Klaytn license",
			pat:  regexp.MustCompile(`//\s*(Copyright (?P<year>\d+) (The klaytn Authors)).*?\n(?://.*?\n)*//.*((>\.)|(See LICENSE in the top directory for the original copyright and license\.))`), // log directory has both patterns
			writer: func(b []byte, matches []int, info *info) []byte {
				buf := new(bytes.Buffer)
				info.originalLicense = string(b[matches[0]:matches[1]])
				modifiedKaiaLicenseT.Execute(buf, info)
				buf.Write(b[matches[1]:])
				return buf.Bytes()
			},
		},
		LICENSE_TYPE_MIXED: {
			name: "Mixed license (Klaytn + Ethereum)",
			pat:  regexp.MustCompile(`//\s*(Modifications Copyright (?P<year>\d+) (The klaytn Authors)).*?\n(?://.*?\n)*// Modified and improved for the klaytn development.`),
			writer: func(b []byte, matches []int, info *info) []byte {
				buf := new(bytes.Buffer)
				info.originalLicense = string(b[matches[0]:matches[1]])
				modifiedKaiaLicenseT.Execute(buf, info)
		...
		LICENSE_TYPE_ETHEREUM: {
			name:   "Ethereum license",
			pat:    regexp.MustCompile(`//\s*(Copyright (?P<year>\d+) (The go-ethereum Authors)).*?\n(?://.*?\n)*//.*>\.`),
			writer: nil,
		},
		LICENSE_TYPE_GO: {
			name:   "Go license",
			pat:    regexp.MustCompile(`//\s*(Copyright (?P<year>\d+) (The Go Authors)).*?\n(?://.*?\n)*`),
			writer: nil,
		},
		LICENSE_TYPE_ANY: {
			name:   "Any license",
			pat:    regexp.MustCompile(`//\s*Copyright*`),
			writer: nil,
		},
		LICENSE_TYPE_NONE: {
			name: "No license",
	...
	ethereumDir = map[string]string{
		...
		"client/kaia_client.go":                              "ethclient/ethclient.go",
		"client/kaia_client_test.go":                         "ethclient/ethclient_test.go",
...
var modifiedKaiaLicenseT = template.Must(template.New("").Parse(`
// Modifications Copyright 2024 The Kaia Authors
{{.OriginalLicense}}
// Modified and improved for the Kaia development.`[1:]))

var kaiaLicenseT = template.Must(template.New("").Parse(`
...
func (i info) OriginalLicense() string {
	return i.originalLicense
}
...
func categorize(content []byte) int {
	for i := 0; i < NUM_LICENSE; i++ {
		r := licenseReplacers[i]
		if r.pat.FindIndex(content) != nil {
			return i
		}
	}
...
func writeLicense(info *info) {
	...
	licenseType := categorize(content)

	// if license type is set, update license only if the file matches the given license type
	if ty := os.Getenv("LICENSE_TYPE"); ty != "" && strconv.Itoa(licenseType) != ty {
		return
	}

	r := licenseReplacers[licenseType]
	r.pat.FindSubmatchIndex(content)
	if r.writer != nil {
		matches := r.pat.FindSubmatchIndex(content)
		newContent := r.writer(content, matches, info)
		if err := os.WriteFile(info.file, newContent, fi.Mode()); err != nil {
			log.Fatalf("error writing %s: %v", info.file, err)
		}
		fmt.Println("Updated", info.file, "license type:", r.name)
	}
}
