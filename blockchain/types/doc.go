...
Kaia nodes achieve a consensus of the global state by processing the same transactions in the same order.

Transaction is an atomic operation originated from an Externally Owned Account (EOA) transiting the global state of Kaia.
Kaia supports multiple transaction types to efficiently trigger KAIA transfer or contract execution.
LegacyTransaction, the Ethereum compatible transaction, could be sent to both of EOA and Smart Contract (SC).
However, it cannot support advanced features of Kaia such as multi-sig or fee-delegation.
...
A block guarantees the sequential execution of transactions inside, and a block header enables Kaia nodes to order the blocks.
...
This package implements Kaia Block, Transaction and related functions and variables to support them.
...
Functions and variables related to Block and Transaction are defined in the files listed below.
  ...
  - tx_internal_data_value_transfer.go: implements the transaction sending KAIA to an EOA
  - tx_internal_data_value_transfer_memo.go: implements the transaction sending KAIA to an EOA with data
